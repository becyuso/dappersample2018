...
namespace dappersampleApi2018.ApiHelper
{
    ...
    {
        public static T GetResponseDataSingle<T>(string api, Dictionary<string,string> conditions)
        {
            return GetResponseDataSingle<T>(GetWebConfigSetting(), api, conditions);
        }

        public static T GetResponseDataSingle<T>(string uri, string api, Dictionary<string, string> conditions)
        {
            return (api.Equals("")) ? default(T) : DeserializeObject<T>(SendRequestToAPI(uri, api, conditions));
        }

        public static List<T> GetResponseData<T>(string api, Dictionary<string, string> conditions)
        {
            return GetResponseData<T>(GetWebConfigSetting(), api, conditions);
        }

        public static List<T> GetResponseData<T>(string uri, string api, Dictionary<string, string> conditions)
        {
        ...
        public static T GetResponseObject<T>(string api, string action, Dictionary<string, string> conditions)
        {
            return DeserializeObject<T>(SendRequestToAPI(GetWebConfigSetting(), api, action, conditions));
        }

        public static List<T> GetResponseList<T>(string api, string action, Dictionary<string, string> conditions)
        {
